...
| **cron 假成功（只有分析报告）** | Agent 把 analysis 当完成，未跑 publish_article | 必须跑 publish_article.js + verify_publish.js；无 media_id 禁止报成功。 |
| **PowerShell ConvertFrom-Json 失败** | 禁止用 PS 读 JSON | 改用 node scripts/fetch_ai_daily.js 或 node -e JSON.parse |
